    ...
    <>
      <section style={createBoxStyle}>
        ...
        <select
          ...
          onChange={(event) => onTargetMemberIdChange(event.target.value)}
          style={inputStyle}
        >
          <option value="">보상 대상 참여자 선택</option>
          {members.map((member) => (
            <option key={member.id} value={member.id}>
              {member.display_name} · 스티커 {balanceByMemberId(member.id)}개
            </option>
          ))}
        </select>
        ...
        <input
          ...
          min={0}
          value={costPoints}
          onChange={(event) => onCostPointsChange(Number(event.target.value))}
      ...
      <section style={rewardListSectionStyle}>
        ...
        {rewards.length === 0 ? (
          <div style={emptyStateStyle}>등록된 보상이 없습니다.</div>
        ) : (
          <div style={rewardListStyle}>
            {rewards.map((reward) => {
              ...
              const canRedeem =
                reward.status !== "redeemed" && balance >= reward.cost_points;
              ...
              return (
                <div key={reward.id} style={rewardCardStyle}>
                  <div>
                    ...
                    <div style={rewardSubTextStyle}>
                      대상:{" "}
                      {memberNameById(members, reward.target_member_id)} · 필요{" "}
                      {reward.cost_points}개 · 현재 {balance}개
                    </div>
                  </div>
                  ...
                  {reward.status === "redeemed" ? (
                    ...
                    <button
                      ...
                      style={
                        canRedeem ? rewardButtonStyle : disabledRewardButtonStyle
                      }
                    >
                      교환하기
                    </button>
...
const rewardListSectionStyle: CSSProperties = {
  marginBottom: 80,
};
...
const rewardSubTextStyle: CSSProperties = {
  marginTop: 5,
  color: "#64748b",
  fontSize: 13,
};
...
const redeemedBadgeStyle: CSSProperties = {
  width: "fit-content",
  padding: "6px 10px",
  borderRadius: 999,
  background: "#dcfce7",
